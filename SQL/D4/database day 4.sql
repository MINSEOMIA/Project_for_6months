...
-- (원하는 결과가 아님!!) 1위인 영화들의 평균 매출액 구하기: 영화한편 또는 두편에 대한 평균이 나옴
...
select year(release_date),movie_name,sale_amt, 
		case when sale_amt > (select avg(sale_amt) as avg_amt from box_office where ranks =1) then sale_amt else '평균 매출액보다 작음' end '평균매출액과 비교'
...
select a.name, a.district, a.population, a.countrycode,
  ( select b.name -- 스칼라 서브쿼리에서는 한컬럼만 나와야 함
      from country b
        where a.countrycode = b.code
  )countryname  -- 한개의 로우/한개의 컬럼-> 단일값
  from city a;
  
  
  select a.name, a.district, a.population, a.countrycode,
  ( select concat(b.name, '/', b.continent) -- 스칼라 서브쿼리에서는 한컬럼만 나와야 함
      from country b
        where a.countrycode = b.code
  )countryname  -- 한개의 로우/한개의 컬럼-> 단일값
  from city a;
  ...
  -- 내부 조인 이용
  select a.name, a.district, a.population, a.countrycode, b.name
   from city a
   join country b
...
-- 해당 영화의 매출액이 전체 매출액에서 차지하는 비율을 구하는 쿼리

 

-- 3. Where 절의 서브쿼리
-- 2019년도 개봉한 영화의 이름과 매출액중 2018년도 최대 매출액보다 큰 영화
  select movie_name, sale_amt
  from box_office
  where year(release_date) = 2019
    and sale_amt >= (select max(sale_amt)
                     from box_office
                       where year(release_date) = 2018); -- 2018년도 최대 매출액
                    
-- where절 다수의값이 나와서 error                    
  select movie_name, sale_amt
  from box_office
  where year(release_date) = 2019
    and sale_amt >= (select sale_amt
                     from box_office
                       where year(release_date) = 2018
                        and ranks between 1 and 3); -- 2018년도 최대 매출액                       
  ...
  where year(release_date) = 2019
    and (movie_name, director) in (select movie_name, director
                        from box_office
                         where year(release_date)=2018);        
-- NOT IN
select movie_name, ranks, release_date, sale_amt, rep_country
  from box_office
  where year(release_date) = 2019
