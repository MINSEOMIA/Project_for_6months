...
-- 이 때 box_office 테이블에서 2019년 개봉 영화 중 관객수가 800만 명 이상인 데이터를 box_office 테이블에 넣는 insert문 작성하기
...
-- box_office 테이블의 2018년 개봉 영화와 순위(ranks)가 같은 건의 관객수(audience_num) 값으로 변경하는 update 문 작성하기
alter table box_office_copy
add column last_year_audi_num int null;

select ranks, audience_num
from box_office b, box_office _copy a
where a.ranks = b.ranks 
 and year(b.release_date) = 2018
 


-- (실습) 사원의 부서 할당 정보가 들어 있는 dept_emp 테이블에서 현재 기준이 아닌 과거 기준으로 데이터를 삭제하는 delete 문 작성




with summary as(
select year(release_Date) years, sum(sale_amt)


select b.years,a.ranks,a.movie

select a.ranks, a.movie_name, a.sale_amt
from box_office a
